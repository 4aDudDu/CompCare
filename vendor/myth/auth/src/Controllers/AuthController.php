...
class AuthController extends Controller
{
    ...
    public function register()
    {
        ...
        $rules = [
            ...
            'no_hp'    => 'required|is_unique[users.no_hp]|max_length[15]'
        ];
        ...
        if (!$this->validate($rules)) {
            return view('Myth\Auth\Views\register', [
                ...
                'config' => new AuthConfig()  // Pastikan mengirimkan objek AuthConfig ke view
            ]);
        }
        ...
        $user->password = password_hash($this->request->getPost('password'), PASSWORD_DEFAULT); // Enkripsi password
    ...
    public function attemptRegister()
    {
        ...
        $rules = config('Validation')->registrationRules ?? [
            ...
            'email'    => 'required|valid_email|is_unique[users.email]',
            'no_hp'    => 'required|regex_match[/^[0-9]{10,15}$/]', // Validate phone number
        ];
        ...
        // Save the user with 'no_hp'
        $allowedPostFields = array_merge(['password'], $this->config->validFields, $this->config->personalFields, ['no_hp']);
